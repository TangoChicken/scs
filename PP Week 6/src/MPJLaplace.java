...
public class MPJLaplace {
	...
	public static void main(String args[]) throws Exception {
		...
		for (int iter = 0; iter < NITER; iter++) {
			...
			int prev = me - 1;
			if (prev >= 0 && next < P) {
				MPI.COMM_WORLD.Sendrecv(phi[B], 0, N, MPI.FLOAT, next, 0, phi[0], 0, N, MPI.FLOAT, prev, 0);
				MPI.COMM_WORLD.Sendrecv(phi[1], 0, N, MPI.FLOAT, prev, 0, phi[B + 1], 0, N, MPI.FLOAT, next, 0);
